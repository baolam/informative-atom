...
class ProblemSolver(HybridUnit, ABC):
    ...
    def _as_object(self):
        """
        Trả về lớp dựng của đối tượng.

        Lớp dựng có hỗ trợ classmethod from_dict
        """
        pass
    ...
    def as_entity(self, *args, **kwargs):
        ...
        """
        raw = self.forward(*args, **kwargs)
        if not self.satisfy_rules(raw):
            raise RuntimeError("Expression does not satisfy rule!")
        
        # Cho phép cập nhật thêm dữ liệu ngoài
        raw.update(**kwargs)

        return self._as_object()(**raw)
